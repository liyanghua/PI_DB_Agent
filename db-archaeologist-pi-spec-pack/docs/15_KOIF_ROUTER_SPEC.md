...
- 仅消费 KDS + TMS 两个分数（其余 6 维 `unavailable`）
- 仅 3 条路由规则、3 类 action（详见 §5、§6）
- 不支持跨 entity 对比
- 不支持时序策略追踪（同 entity 多次 router_run 之间的策略变化）
...
Phase 3+ 按相同模式扩展 PVS / CES / PFS / NOS / BDS / CPS。
...
- `paid_test`：取 KDS ≥ 80 & TMS ≥ 60 交集 TOP N（两者交集）
